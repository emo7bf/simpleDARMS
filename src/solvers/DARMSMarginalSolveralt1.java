...
public class DARMSMarginalSolveralt1{
	...
	public DARMSMarginalSolveralt1(DARMSModel model, boolean zeroSum, boolean decomposed, boolean flightByFlight, boolean naive) throws Exception{
		this.model = model;
	...
	public DARMSMarginalSolveralt1(DARMSModel model, PassengerDistribution passengerDistribution, boolean zeroSum, boolean decomposed, boolean flightByFlight, boolean naive) throws Exception{
		this.model = model;
		this.xiDistribution = model.getXiDistribution();
		this.payoffStructure = model.getPayoffStructure();
		this.zeroSum = zeroSum;
	...
	public DARMSMarginalSolveralt1(DARMSModel model, PayoffStructure payoffStructure, boolean zeroSum, boolean decomposed, boolean flightByFlight, boolean naive) throws Exception{
		this.model = model;
		this.xiDistribution = model.getXiDistribution();
				...
				for(Flight f : model.getFlights(t)){
					...
					for(RiskCategory c : model.getAdversaryDistribution().keySet()){
						...
						for(ScreeningOperation o : model.getScreeningOperations()){
							IloNumVar var = cplex.numVar(-MM, MM, IloNumVarType.Float, "m_t" + t + "_i" + it + "_f" +  f.id() + "_c" + c.toString() + "_o" + o.getID());
